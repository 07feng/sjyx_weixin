...
public class MyBeanUtils extends org.apache.commons.beanutils.BeanUtils {
	...
	/**
	 * 对象拷贝 数据对象空值不拷贝到目标对象 [时间未赋值 已经修复]
	 * 
	 * @param databean
	 * @param tobean
	...
	public static void copyBeanNotNull2Bean(Object databean, Object tobean) throws Exception {
		if(databean == null || tobean == null){
			throw new Exception("[更新]copy对象不能为空");
		}
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			if (PropertyUtils.isReadable(databean, name) && PropertyUtils.isWriteable(tobean, name)) {
				try {
					...
					if (value != null) {
						if (name.contains("Time") || name.contains("Birthday") || name.contains("Date")) {
							try {
