...
/**
 * filmfestivalvip_comment_fid 返回数据的加载
 * 
 * @author 强强
 *
 */
public class Filmfestivalvip_comment_fidUtil {
	 ...
	 * @return
	 */
	public static List getControllerList(List<Filmfestivalvip_comment_fid> list) {
		List item = new ArrayList();
		for (Filmfestivalvip_comment_fid obj : list) {
	...
	public static List getCList(List<Filmfestivalvip_comment_fid> list) {
		List item = new ArrayList();
		for (Filmfestivalvip_comment_fid obj : list) {
			Map<Object, Object> map = new HashMap<Object, Object>();
			...
			map.put("id", obj.getId());
			if (obj.getFd_doccomment_id() >= 0) {
				map.put("fd_doccomment_id", obj.getFd_doccomment_id());
			} else {
				map.put("fd_doccomment_id", "");
			}
			if (obj.getFd_comment_time() != null) {
				map.put("fd_comment_time", obj.getFd_comment_time());
			} else {
			...
			if (obj.getFd_comment_note() != null) {
				map.put("fd_comment_note", obj.getFd_comment_note());
			} else {
				map.put("fd_comment_note", "");
			}
			item.add(map);
		}
		return item;
	 ...
	 * @return
	 */
	public static Map getControllerMap(Filmfestivalvip_comment_fid obj) {
		Map<Object, Object> map = new HashMap<Object, Object>();
		...
		if (obj.getFd_member_id() != null) {
			map.put("fd_member", obj.getFd_member_id());
		} else {
			map.put("fd_member", "");
		}
		if (obj.getFd_c_member_id() != null) {
			map.put("fd_c", obj.getFd_c_member_id());
		} else {
			map.put("fd_c", "");
		}
		if (obj.getFd_comment_time() != null) {
	 ...
	 * @return
	 */
	public static List getControllerListAll(
			List<Filmfestivalvip_comment_fid> list) {
		List item = new ArrayList();
		for (Filmfestivalvip_comment_fid obj : list) {
			Map<Object, Object> map = new HashMap<Object, Object>();
